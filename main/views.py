    ...
    if req.method == 'GET':
        ...
        try:
            ...
            if len(result) < 1:
                ...
                # 专业代码
                specialized_course = specialized_course_tr.find_all('td')[0].get_text()
                ...
                if '085400' in st_type:
                    if '961' in specialized_course:
                        student_type = 0
                    else:
                        student_type = 2
                # 学硕
                elif '083500' in st_type:
                    student_type = 1
                else:
                    student_type = 2
                ...
                if student_type != 2:
                    # 修改所用数据库为sqlite3，不用再安装mysql
                    connect = _sqlite3.connect('data_rank.db')
                    cursor = connect.cursor()
                    sql = "SELECT * FROM student WHERE number = %s;"
                    cursor.execute(sql % int(username))
                    if cursor.rowcount > 0:
                        pass
                    else:
                        #主键唯一性原则 需加or IGNORE
                        sql = "INSERT or IGNORE INTO student(number, type, grade) VALUES (%s, %d, %d);"
                    ...
                    if student_type == 0:
                        ...
                        #sqlite3写法
                        total = cursor.execute("SELECT COUNT(*) FROM student").fetchone()[0]
